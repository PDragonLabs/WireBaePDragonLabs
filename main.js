...
let dragonfly, particles, audio, analyser, filter, targetFrequency, gridHelper, pyramids, stars, spawnedObjects, vibePinGroup;
...
const kickVolumeSlider = document.getElementById('kick-volume-slider');
const hatVolumeSlider = document.getElementById('hat-volume-slider');
const noteVolumeSlider = document.getElementById('note-volume-slider');
...
const hint3Ok = document.getElementById('hint3-ok');
...
function init() {
    ...
    vibePinGroup = new THREE.Group();
    scene.add(spawnedObjects, vibePinGroup);
...
function setupSynths() {
    ...
    synths.kickVolume = new Tone.Volume(0).toDestination();
    synths.hatVolume = new Tone.Volume(0).toDestination();
    synths.noteVolume = new Tone.Volume(0).connect(reverb);
    synths.padVolume = new Tone.Volume(0).connect(reverb);

    synths.kick = new Tone.MembraneSynth({ pitchDecay: 0.05, octaves: 10, oscillator: { type: "sine" }, envelope: { attack: 0.001, decay: 0.4, sustain: 0.01, release: 1.4, attackCurve: "exponential" } }).connect(synths.kickVolume);
    synths.hat = new Tone.MetalSynth({ frequency: 200, envelope: { attack: 0.001, decay: 0.1, release: 0.01 }, harmonicity: 5.1, modulationIndex: 32, resonance: 4000, octaves: 1.5 }).connect(synths.hatVolume);
    synths.note = new Tone.FMSynth({ harmonicity: 3, modulationIndex: 10, detune: 0, oscillator: { type: "sine" }, envelope: { attack: 0.01, decay: 0.1, sustain: 0.5, release: 1 }, modulation: { type: "square" }, modulationEnvelope: { attack: 0.5, decay: 0, sustain: 1, release: 0.5 } }).connect(synths.noteVolume);
    synths.pad = new Tone.PolySynth(Tone.AMSynth, { harmonicity: 1.5, detune: 0, oscillator: { type: "fatsawtooth" }, envelope: { attack: 0.1, decay: 0.2, sustain: 0.5, release: 1.2 }, modulation: { type: "sine" }, modulationEnvelope: { attack: 0.5, decay: 0, sustain: 1, release: 0.5 } }).connect(synths.padVolume);
}
...
function setupEventListeners() {
    ...
    kickVolumeSlider.addEventListener('input', e => { if (synths.kickVolume) synths.kickVolume.volume.value = parseFloat(e.target.value); });
    hatVolumeSlider.addEventListener('input', e => { if (synths.hatVolume) synths.hatVolume.volume.value = parseFloat(e.target.value); });
    noteVolumeSlider.addEventListener('input', e => { if (synths.noteVolume) synths.noteVolume.volume.value = parseFloat(e.target.value); });
    padVolumeSlider.addEventListener('input', e => { if (synths.padVolume) synths.padVolume.volume.value = parseFloat(e.target.value); });
    ...
    sequencerToggleButton.addEventListener('click', () => {
        if (Tone.Transport.state === 'started') {
            ...
            playhead.visible = false;
            sequencerToggleButton.textContent = 'Resume Sequencer';
        } else {
    ...
    // --- MODIFIED: `clearSceneButton` Event Listener ---
    clearSceneButton.addEventListener('click', () => {
        // Clear spawned sound objects
        ...
        // Clear vibe pin 3D objects
        while(vibePinGroup.children.length > 0){
            const pin = vibePinGroup.children[0];
            vibePinGroup.remove(pin);
        }
        // Clear vibe pin data array
        vibePins = [];
        renderPinList();

        // **THE FIX**: Reset the vibe pin state variable
        isVibePinned = false;
        
        // **UX Improvement**: If the sequencer was paused (likely by a pin), resume it.
    ...
    pinList.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('goto-btn')) {
            ...
            removePin(parseInt(target.dataset.id));
        }
    });
...
function onSceneClick(event) {
    ...
    if (pinIntersects.length > 0) {
        const clickedPin = pinIntersects[0].object.parent;
        if (clickedPin.userData.isActive) {
            isVibePinned = false;
            removePin(clickedPin.userData.id);
        } else {
            goToPin(clickedPin.userData.id);
        }
    } else if (currentObjectToAdd) {
        ...
        isVibePinned = true;
        vibePinGroup.children.forEach(pin => pin.userData.isActive = false);
        const newPinData = {
            id: Date.now(),
            rate: audio.getPlaybackRate(),
            freq: targetFrequency
        };
        vibePins.push(newPinData);
        createVibePin(newPinData);
        renderPinList();
        if (Tone.Transport.state === 'started') {
            Tone.Transport.pause();
            sequencerToggleButton.textContent = 'Resume Sequencer';
...
function createTextSprite(message) {
    ...
    context.font = "Bold 40px Rajdhani";
...
function createVibePin(pinData) {
    ...
    const textSprite = createTextSprite(`Pin ${vibePins.length}`);
    textSprite.position.y = 2;
    
    pinGroup.add(pin);
    pinGroup.add(textSprite);
    ...
    pinGroup.userData = { ...pinData, isActive: true };
    vibePinGroup.add(pinGroup);
}
...
function updateAudioFromMouse() {
    ...
    if (audio) {
        ...
        const newRate = minRate + ((mouse.x + 1) / 2) * (maxRate - minRate);
        audio.setPlaybackRate(newRate);
    }
}

function goToPin(id) {
    const pinData = vibePins.find(p => p.id === id);
    if (pinData) {
        isVibePinned = true;
        vibePinGroup.children.forEach(pinObj => {
            pinObj.userData.isActive = pinObj.userData.id === id;
        });
        targetFrequency = pinData.freq;
        audio.setPlaybackRate(pinData.rate);
        if (Tone.Transport.state === 'started') {
            Tone.Transport.pause();
            sequencerToggleButton.textContent = 'Resume Sequencer';
        }
...
function removePin(id) {
    vibePins = vibePins.filter(p => p.id !== id);
    const pinToRemove = vibePinGroup.children.find(p => p.userData.id === id);
    if (pinToRemove) {
        vibePinGroup.remove(pinToRemove);
    }
    renderPinList();
    isVibePinned = false;
    if (Tone.Transport.state === 'paused') {
...
function renderPinList() {
    pinList.innerHTML = '';
    vibePins.forEach((pin, index) => {
        const li = document.createElement('li');
        li.textContent = `Pin ${index + 1}`;
        
        const goToBtn = document.createElement('button');
        goToBtn.textContent = 'Go to';
        goToBtn.className = 'goto-btn';
        goToBtn.dataset.id = pin.id;

        const removeBtn = document.createElement('button');
        removeBtn.textContent = 'Remove';
        removeBtn.className = 'remove-btn';
        removeBtn.dataset.id = pin.id;
...
function startExperience() {
    ...
    Tone.start().then(() => {
        const threeAudioContext = audio.context;
        if (threeAudioContext.state === 'suspended') {
            threeAudioContext.resume().then(activate).catch(err => console.error("Error resuming Three.js AudioContext:", err));
        } else {
            activate();
        }
    }).catch(err => console.error("Error starting Tone.js AudioContext:", err));
}
...
function animate() {
    ...
    vibePinGroup.children.forEach(pin => {
        pin.children[0].material.color.set(pin.userData.isActive ? 0x00ffff : 0xff00ff);
