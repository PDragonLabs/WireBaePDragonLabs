...
const pinVibeButton = document.getElementById('pin-vibe-btn');
...
function setupEventListeners() {
    ...
    pinVibeButton.addEventListener('click', toggleVibePin);
...
function toggleVibePin() {
    isVibePinned = !isVibePinned;
    if (isVibePinned) {
        pinVibeButton.textContent = 'Unlock Vibe';
        const newPin = {
            id: Date.now(),
            x: mouse.x,
            y: mouse.y,
        ...
        vibePins.push(newPin);
        renderPinList();
    } else {
        pinVibeButton.textContent = 'Set Vibe Pin';
    }
}

function goToPin(id) {
    const pin = vibePins.find(p => p.id === id);
    if (pin) {
        isVibePinned = true;
        pinVibeButton.textContent = 'Unlock Vibe';
        targetFrequency = pin.freq;
        audio.setPlaybackRate(pin.rate);
    }
}
...
function renderPinList() {
    ...
    vibePins.forEach((pin, index) => {
        ...
        goToBtn.onclick = () => goToPin(pin.id);

        const removeBtn = document.createElement('button');
        removeBtn.textContent = 'Remove';
        removeBtn.onclick = () => removePin(pin.id);
