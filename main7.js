...
function onSceneClick(event) {
    ...
    const pinIntersects = raycaster.intersectObjects(vibePinGroup.children);

    if (pinIntersects.length > 0) {
        // Clicked on an existing pin
        const clickedPin = pinIntersects[0].object;
        if (clickedPin.userData.isActive) {
            // Clicked the active pin, so unlock
            isVibePinned = false;
            vibePinGroup.remove(clickedPin);
        } else {
            // Clicked an inactive pin, so switch to it
            vibePinGroup.children.forEach(pin => pin.userData.isActive = false);
            clickedPin.userData.isActive = true;
            targetFrequency = clickedPin.userData.freq;
            audio.setPlaybackRate(clickedPin.userData.rate);
        }
    } else if (currentObjectToAdd) {
        // Placing a sequencer object
        const intersects = raycaster.intersectObject(groundPlane);
        if (intersects.length > 0) {
            spawnObject(intersects[0].point);
        }
    } else {
        // Setting a new vibe pin
        isVibePinned = true;
        vibePinGroup.children.forEach(pin => pin.userData.isActive = false);
        const newPin = createVibePin();
        newPin.userData.isActive = true;
        newPin.userData.rate = audio.getPlaybackRate();
        newPin.userData.freq = targetFrequency;
        vibePinGroup.add(newPin);
    }
}

function createVibePin() {
    const geometry = new THREE.CylinderGeometry(0.5, 0, 2, 8);
    const material = new THREE.MeshBasicMaterial({ color: 0xff00ff });
    const pin = new THREE.Mesh(geometry, material);
    
    // Position the pin in front of the camera based on mouse click
...
function startExperience() {
    if (isExperienceActive) {
        console.log("Experience is already active. Aborting.");
        return;
    }
    console.log("Activation button clicked. Starting experience...");

    // This function will be called after audio contexts are ready.
    const activate = () => {
        console.log("Activating UI and loading audio.");
    ...
    Tone.start().then(() => {
        ...
        // Resume Three.js AudioContext if needed
        if (threeAudioContext.state === 'suspended') {
            threeAudioContext.resume().then(() => {
                console.log("Three.js AudioContext resumed successfully.");
                activate();
            }).catch(err => {
                console.error("Error resuming Three.js AudioContext:", err);
                aiText.textContent = "Audio activation failed.";
            });
        } else {
            activate();
        }
    }).catch(err => {
        console.error("Error starting Tone.js AudioContext:", err);
        aiText.textContent = "Audio activation failed.";
    });
}
...
function animate() {
    ...
    vibePinGroup.children.forEach(pin => {
        pin.material.color.set(pin.userData.isActive ? 0x00ffff : 0xff00ff);
    });
